 ...
 * You will be given a sequence of passages, and must filter out any passage whose text (sequence of 
 * whitespace-delimited words) is wholly contained as a sub-passage of one or more of the other passages.
 ...
 * Duplicates must also be filtered - if two passages are considered equal with respect to the comparison 
 * rules listed above, only the shortest should be retained.
 * If they are also the same length, the earlier one in the input sequence should be kept. The retained 
 * passages should be output in their original form (identical to the input passage),and in the same order.
 * Input:
 * 
